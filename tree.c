...
void printName( )
{
    ...
    printf("\nThis solution was completed by: Qustandi Fashho\n");
    printf("NA\n\n");
}
    ...
    while(x != NULL){ // while x is not null
        ...
        if((balance <= -2) || (balance >= 2)){ // this is if the rank is not -1,1,0 (should I change to that??) 
            ...
            if((balance > 0 && getBalance(z) < 0) || (balance < 0 && getBalance(z) > 0)){ // if the balance of x and z have different signs 
                if(getBalance(z) > 0){
                    rightRotate(t, z); // balance is positive so right rotate 
                }
                else{
                    leftRotate(t,z); // balance is negative so left rotate 
                }
        ...
        x = x->pParent; // set x equal to the parent of x 
    }
...
TNode* getTallerSubTree(TNode* root){ // FINISH THIS NEED IT FOR FUNCTION ABOVE
    ...
    if(root == NULL){
        ...
        if(root->pLeft != NULL){
            heightOfLeftBranch = root->pLeft->height;
        }
        else{
            heightOfLeftBranch = -1;
        }
        if(root->pRight != NULL){
            heightOfRightBranch= root->pRight->height;
        }
        else{
            heightOfRightBranch = -1;
        }

        if(heightOfLeftBranch > heightOfRightBranch){ // if the branch of the left side is taller 
            return root->pLeft;
        }
        else if(heightOfLeftBranch < heightOfRightBranch){
            return root->pRight;
        }
        else{
            return NULL;
...
void printHuffmanEncoding( TNode* root, char c ){
    ...
    if (strchr(root->str, c) != NULL) {
        ...
        if (leftChild && strchr(leftChild->str, c) != NULL) {
            printf("0");
            printHuffmanEncoding(leftChild, c);
        } 
        else if (rightChild && strchr(rightChild->str, c) != NULL) {
            printf("1");
            printHuffmanEncoding(rightChild, c);
        }
    } else {
...
// DONT NEED TO DO ANYTHING BELOW HERE 
